...
class TestTextNodeToHTMLNode(unittest.TestCase):
    ...
    def test_image(self):
        node = TextNode("This is alt text for an image text node", TextType.IMAGE, "image_url")
        html_node = text_node_to_html_node(node)
        self.assertEqual(html_node.tag, "img")
        self.assertEqual(html_node.value, "")
        self.assertEqual(html_node.props, {"src": "image_url", "alt": "This is alt text for an image text node"})

    def test_unsupported_text_type(self):
        node = TextNode("This is an unsupported text type node", "text")
